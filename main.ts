...
async function callGemini(
  ...
  if (imageContent) return { type: "image", content: imageContent };
  if (textContent.trim()) return { type: "text", content: textContent.trim() };
  ...
  if (pathname.includes(":streamGenerateContent")) {
    try {
      ...
      if (lastUserMessageIndex !== -1) {
        const lastModelMsgIndex = fullHistory.findLastIndex(
          (msg: any, idx: number) => msg.role === "model" && idx < lastUserMessageIndex,
        );
        relevantHistory = fullHistory.slice(
          lastModelMsgIndex === -1 ? 0 : lastModelMsgIndex,
        ...
        async start(controller) {
          ...
          try {
            ...
            if (geminiResult.type === "image") {
              const [mimeType, base64Data] = geminiResult.content
                .split(/data:(.+);base64,(.*)/)
                .filter(Boolean);
              sendChunk({
                candidates: [
